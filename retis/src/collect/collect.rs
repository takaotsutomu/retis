...
impl Collectors {
    ...
    fn init_distributed(&mut self, collect: &Collect) -> Result<()> {
        ...
        if let Some(ref addr) = collect.aggregator {
            ...
            let collector =
                DistributedCollector::new(config, self.node_identity.as_ref().unwrap().clone());
    ...
    fn enrich_event(&mut self, event: &mut Event) -> Result<()> {
        ...
        let (ntp_offset_ns, ntp_uncertainty_ns, sync_status) =
            if let Some(ref mut monitor) = self.ntp_monitor {
                match monitor.get_status() {
                    Ok(status) => {
                        ...
                        (status.offset_ns, status.uncertainty_ns, sync)
                    }
                    Err(_) => (0, 0, SyncStatus::Unsynchronized),
                }
            } else {
                (0, 0, SyncStatus::Unsynchronized)
            };
