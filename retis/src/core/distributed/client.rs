...
pub struct DistributedCollectorConfig {
    ...
    /// Maximum events to buffer in worker when paused/disconnected.
...
impl CollectorWorker {
    ...
    fn run(mut self) {
        ...
        loop {
            ...
            if let Err(e) = self.check_for_control_messages() {
                debug!("Error checking control messages: {}", e);
            }
        }
    }
    ...
    fn flush(&mut self) -> Result<()> {
        ...
        match self.recv_message() {
            Ok(response) => match response.payload {
                Payload::BatchAck(ack) => {
                    ...
                    match ack.status {
                        ...
                        BatchStatus::PartialFailure => {
                            warn!(
                                "Batch {} partial failure: {} processed, {} failed",
                                batch_id, ack.events_processed, ack.events_failed
                            );
                        }
                ...
                Payload::Pause(pause) => {
                    info!("Aggregator requested pause: {:?}", pause.reason);
                    self.paused = true;
                }
                _ => {
    ...
    fn check_for_control_messages(&mut self) -> Result<()> {
        if self.stream.is_none() {
            return Ok(());
        }
        ...
        match result {
            Ok(msg) => match msg.payload {
                Payload::Resume(resume) => {
                    info!("Aggregator sent Resume");
                    self.paused = false;
                    if let Some(recommended) = resume.recommended_batch_size {
                        if recommended as usize != self.config.batch_size {
                            debug!(
                                "Updating batch_size from {} to {} (aggregator recommendation)",
                                self.config.batch_size, recommended
                ...
                _ => {
                    debug!("Unexpected control message: {:?}", msg.payload);
                }
            },
            Err(e) => {
                let err_str = e.to_string();
                if !err_str.contains("WouldBlock") && !err_str.contains("os error 11") {
                    debug!("Error checking for control messages: {}", e);
                }
            }
        }
...
impl DistributedCollector {
    pub fn new(config: DistributedCollectorConfig, identity: NodeIdentity) -> Self {
...
impl Drop for DistributedCollector {
    fn drop(&mut self) {
        let _ = self.sender.send(CollectorCommand::Shutdown);

        if let Some(thread) = self.thread.take() {
            if let Err(e) = thread.join() {
                warn!("Failed to join collector worker: {:?}", e);
            }
        }
    }
}
