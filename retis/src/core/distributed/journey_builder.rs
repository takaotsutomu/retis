//! Journey builder for constructing packet journeys from ClickHouse events.
//!
...
use super::query::{ClickHouseQueryClient, EventQueryFilter, EventQueryRow};
...
pub(crate) struct JourneyBuilder<'a> {
    client: &'a ClickHouseQueryClient,
}

impl<'a> JourneyBuilder<'a> {
    pub fn new(client: &'a ClickHouseQueryClient) -> Self {
        Self { client }
    }
    ...
    /// Assumes events are already ordered by `epoch_ns` (from ClickHouse ORDER BY).
...
fn row_to_hop(row: &EventQueryRow) -> Result<JourneyHop> {
    Ok(JourneyHop {
        ...
        uncertainty_ns: row.ntp_uncertainty_ns,
