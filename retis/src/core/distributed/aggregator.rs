...
impl EventSink for LoggingEventSink {
    ...
    fn flush(&mut self) -> Result<()> {
        debug!("LoggingEventSink: {} total events processed", self.events_processed);
        Ok(())
    }
}
...
impl ConnectionHandler {
    ...
    fn handle_event_batch(&mut self, batch: EventBatch) -> Result<()> {
        ...
        if !self.registered {
            warn!("Session {} not registered, rejecting batch", self.session_id);
            let ack = BatchAck {
...
mod tests {
    ...
    #[test]
    fn test_logging_event_sink() {
        let mut sink = LoggingEventSink::new();
    ...
    #[test]
    fn test_session_registration_and_ack() {
        ...
        let running = Arc::clone(&aggregator.running);
        ...
        let register = Register {
            node_id: [1u8; 16],
            node_name: "test-collector".to_string(),
        ...
        let msg = Message::new(0, 0, Payload::Register(register));
