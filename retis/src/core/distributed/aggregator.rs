...
pub(crate) struct AggregatorConfig {
    ...
    /// I/O timeout for read/write operations.
...
pub(crate) trait EventSink: Send {
    /// Returns the status to send back to the collector.
    ...
    fn handle_event_batch(&mut self, batch: EventBatch) -> Result<()> {
        ...
        if let Ok(mut sessions) = self.sessions.write() {
            if let Some(session) = sessions.get_mut(&self.session_id) {
                session.events_received += event_count as u64;
                session.batches_received += 1;
            }
        }

        let events_processed = match &status {
            BatchStatus::Accepted => event_count,
            BatchStatus::Rejected(_) => 0,
        };
        let events_failed = event_count - events_processed;
...
impl TraceAggregator {
    ...
    ///
    /// Set the `shutdown` flag to `true` to stop the accept loop
